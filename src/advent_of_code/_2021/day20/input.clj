(ns advent-of-code.-2021.day20.input
  (:require [clojure.string :refer [trim split]]))

(defn fmt-pixels [chrs]
  (->> chrs vec)
  #_(->> chrs (mapv #(-> % str keyword))))

(defn fmt-algo [raw & [multiline?]]
  (if multiline?
    (->> (split raw #"\n") (map trim) (apply concat) fmt-pixels)
    (->> raw trim fmt-pixels)))
...
(defn fmt [raw & [multiline-algo?]]
  (let [[algo-raw img-raw] (split raw #"\n\n")]
    [(fmt-algo algo-raw multiline-algo?)
     (fmt-image img-raw)]))

(def input-raw 
  (-> "./resources/_2021/day20/input.txt" slurp trim))
...
(def sample (fmt sample-raw :multiline))
(def input (fmt input-raw))

(def sample-algo (first sample))
(def sample-img0 (second sample))
(def sample-img1 (fmt-image sample-img1-raw))
(def sample-img2 (fmt-image sample-img2-raw))
(def sample-img3 (fmt-image sample-img3-raw))
...
;; pixels centered on the corresponding input image pixel. So, to determine 
;; the value of the pixel at (5,10) in the output image, nine pixels from the 
;; input image need to be considered: 
