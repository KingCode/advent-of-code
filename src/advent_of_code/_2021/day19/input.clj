(ns advent-of-code.-2021.day19.input
  (:require [clojure.string :refer [trim split]]))

;; --- Day 19: Beacon Scanner ---

;; As your probe drifted down through this area, it released
;; an assortment of beacons and scanners into the water. It's
;; difficult to navigate in the pitch black open waters of the
;; ocean trench, but if you can build a map of the trench using
;; data from the scanners, you should be able to safely reach
;; the bottom.

;; The beacons and scanners float motionless in the water;
;; they're designed to maintain the same position for long periods
;; of time. Each scanner is capable of detecting all beacons in a
;; large cube centered on the scanner; beacons that are at most 1000
;; units away from the scanner in each of the three axes
;; (x, y, and z) have their precise position determined relative
;; to the scanner. However, scanners cannot detect other scanners.
;; The submarine has automatically summarized the relative positions
;; of beacons detected by each scanner (your puzzle input).

(def input-raw 
  (-> "./resources/_2021/day19/input.txt" slurp trim))
...
;; that both scanners detect within the overlap. By establishing 12 common
;; beacons, you can precisely determine where the scanners are relative to
;; each other, allowing you to reconstruct the beacon map one scanner at
;; a time.
...
;; Drawing x increasing rightward, y increasing upward, scanners 
;; as S, and beacons as B, scanner 0 detects this:
...
;; same beacons, you can assemble the entire map. For example,
...
(def sample-s0s1-0-overlap-raw ;; var naming scheme: s0s1 -> overlap 
                              ;; beacons b/w s0 and s1 as seen from s0
"-618,-824,-621
-537,-823,-458
-447,-329,318
404,-588,-901
...
;; Because of this, scanner 1 must be at 68,-1246,-43
;; (relative to scanner 0).
(def sample-s1-0-position ;;naming scheme s1-0 = 
                           ;;position of s1 relative to s0 
  [68 -1246 -43])

;; Scanner 4 overlaps with scanner 1; the 12 beacons they both detect
;; (relative to scanner 0) are:

(def sample-s1s4-0-overlap-raw
"459,-707,401
-739,-1745,668
-485,-357,347
432,-2009,850
...
(def sample-s2-0-position
  [1105 -1205 1229])

(def sample-s3-0-position
  [-92 -2380 -20])

;; The full list of beacons (relative to scanner 0) is:
(def sample-beacons-0-position-raw
"-892,524,684
-876,649,763
-838,591,734
-789,900,-551
...
(defn fmt-beacons [lines]
  (->> lines
       (map trim)
       (mapv fmt-beacon)))

(defn fmt-scanner [raw]
  (-> raw trim 
      (split #"---\s+scanner\s+|\s+---")
      last 
      (#(Integer/parseInt %))))
...
(defn fmt-report [raw]
  (let [[hdr lines]
    ...
    [hdr lines]
    [(fmt-scanner (first hdr))
     (fmt-beacons lines)]))


(defn fmt-reports [raw]
  (->> (split raw #"\n\n")
       (map fmt-report)))

(def sample-2d (fmt-reports sample-2d-raw))
(def sample-transforms (->> sample-transforms-raw
                            fmt-reports
                            (map second)))
(def sample (fmt-reports sample-raw))
(def input (fmt-reports input-raw))


(defn fmt-beacons-str [raw]
  (-> raw (split #"\n") fmt-beacons))

(def sample-s0s1-0-overlap 
  (fmt-beacons-str sample-s0s1-0-overlap-raw))

(def sample-s0s1-1-overlap 
  (fmt-beacons-str sample-s0s1-1-overlap-raw))

(def sample-s0s1-overlap (zipmap 
                          sample-s0s1-0-overlap
                          sample-s0s1-1-overlap))

(def sample-s1s4-0-overlap 
  (fmt-beacons-str sample-s1s4-0-overlap-raw))

(def sample-beacons-0-position
  (fmt-beacons-str sample-beacons-0-position-raw))
...
(def sample-beacons-0-max-scanner-pos-sId 2)
(def sample-beacons-0-max-scanner-pos [1105 -1205 1229])
(def sample-beacons-0-min-scanner-pos-sId 3)
(def sample-beacons-0-min-scanner-pos [-92 -2380 -20])
(def sample-max-scanner-manhattan-distance 3621)
