(ns advent-of-code.-2021.day17.input
...
;; Could the keys have fallen into it? You'd better send a probe
;; to investigate.
...
;; if negative) directions. For example, an initial x,y velocity 
;; like 0,10 would fire the probe straight up, while an initial velocity
...
;;     that is, it decreases by 1 if it is greater than 0, increases
...
(def input-raw 
  (-> "./resources/_2021/day17/input.txt" slurp trim))
...
;; 20 and at most 30, and the probe's y position is at least -10 and 
;; at most -5.
...
(def sample-7-2-trajectory [[0 0] [7 2] [13 3] [18 3] [22 2] [25 0]
                            [27 -3] [28 -7]])
...
;; In the bottom right, positions that are within the target area are 
;; shown as T. After each step (until the target area is reached),
;; the position of the probe is marked with #. (The bottom-right # is both
...
(def sample-9-0-trajectory
  [[0 0] [9 0] [17 -1] [24 -3] [30 -6]])
...
;; (Any higher initial y velocity causes the probe to overshoot the target 
;; area entirely.)
...
(def sample (->> sample-raw fmt (into {})))
(def input (->> input-raw fmt (into {})))
...
;; eventually that meet these criteria:

(def sample-initial-velocities-raw
...
(defn fmt-velocity [raw]
  (->> (split raw #",")
       (map trim)
       (mapv #(Integer/parseInt %))))

(def sample-initial-velocities
  (->> (split sample-initial-velocities-raw #"\s+")
       (map fmt-velocity)))
